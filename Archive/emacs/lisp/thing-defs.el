...
;; (def-ts-thing 'ts-defun "")

(put 'ts-defun 'forward-op #'treesit-end-of-defun)
...
(defun better-thing-chars (thing char not-char)
  (put thing 'forward-op
       (lambda (&optional n)
         (let ((n (or n 1))
               (current-prefix-arg nil))
           (if (> n 0)
               (re-search-forward (concat char "+")
                                  nil nil n)
             (while (not (= n 0))
               (when (looking-back not-char (1- (point)))
                 (re-search-backward char)
                 (goto-char (match-end 0))) 
               (if (re-search-backward not-char nil t)
                   (goto-char (match-end 0))
                 (beginning-of-buffer))
               (setq n (1+ n))))
           ))))

(better-thing-chars 'non-space "[^[:space:]\n]" "[[:space:]\n]")
(better-thing-chars 'space "[[:space:]\n]" "[^[:space:]\n]")
(better-thing-chars 'digits "[[:digit:]]" "[^[:digit:]]")
...
(defvar -empty-line-re "^[[:blank:]]*$")


(defun -bopp ()
  (and (bolp)
       (not (looking-at -empty-line-re))
       (or (bobp)
           (save-excursion
             (forward-line -1)
...
(defun -forward-paragraph (&optional n)
  (let ((n (or n 1)))
    (if (< n 0) (-backward-paragraph (- n))
      (forward-line 0)
      (let ((prev-line-is-empty (looking-at -empty-line-re)))
        (while (not (or (= n 0) (eobp)))
          (forward-line +1)
          (let ((this-line-is-empty (looking-at -empty-line-re)))
            (when (and this-line-is-empty (not prev-line-is-empty))
              (setq n (1- n)))
            (setq prev-line-is-empty this-line-is-empty)))
        (when (> n 0)
          (error "no next paragraph"))
        ))))

(defun -backward-paragraph (&optional n)
  (let ((n (or n 1)))
    (if (< n 0) (-forward-paragraph (- n))
      ;; The plan is to traverse lines until we moved from a non-empty line to an empty
      ;; one `N' times and then correct as we overshot a bit. If already at the boundary
      ;; when going backwards, we hence have to do this once more.
      (when (-bopp)
        (setq n (1+ n)))
      (forward-line 0)
      (let ((prev-line-is-empty (looking-at -empty-line-re)))
        (while (not (or (= n 0) (bobp)))
          (forward-line -1)
          (let ((this-line-is-empty (looking-at -empty-line-re)))
            (when (and this-line-is-empty (not prev-line-is-empty))
              (setq n (1- n)))
            (setq prev-line-is-empty this-line-is-empty)))
        ;; Correct for overshooting the target
        (pcase `(,n ,(bobp))
          (`(,0 ,_) (forward-line))
          (`(1 t))
          (`(,_ ,_) (error "no previous paragraph")))))))

(put 'paragraph 'forward-op #'-forward-paragraph)


(defun -forward-defun (&optional n)
  (cond ((> n 0) (end-of-defun n))
        ((< n 0) (beginning-of-defun (- n)))))
(put 'defun 'forward-op #'-forward-defun)

;; (put 'sexp 'bounds-of-thing-at-point-op (lambda (&rest args)
;;                                     (ignore-errors
;;                                       ())))
...
(put 'empty 'forward-op #'forward-char)
(put 'empty 'bounds-of-thing-at-point (lambda ()
                                        (cons (point) (point))))


(with-eval-after-load 'viper
  (put 'viper-word 'forward-op #'viper-forward-word)
  (put 'viper-Word 'forward-op #'viper-forward-Word))
